...
const Profile: React.FC = () => {
    ...
    const updateUserInfo = useCallback(async (data: UpdateUserInfo, version: number, id: string) => {
        showLoader();
        try {
            const { body } = await customerService.updateCustomer(id, version, data);
            setCustomer({
            ...
            enqueueSnackbar(
                `Ошибка обновления данных пользователя: ${message}`,
                { variant: 'error' },
            );
            hideLoader()
        }
    ...
    const changePassword = useCallback(async (data: IChangePasswordForm, version: number, id: string) => {
        showLoader();
        try {
            ...
            const { body } = await customerService.login({email: user!.email, password: data.newPassword});
            login(body.customer);
            hideLoader()
        } catch (e: unknown) {
            const {message} = e as BadRequest;
    ...
    useEffect(() => {
        const isFormChanged = JSON.stringify(watchedValues) !== JSON.stringify(customer);
    ...
    const changePasswordSubmit: SubmitHandler<IChangePasswordForm> = async (data) => {
            ...
            <div className={styles.profile}>
                ...
                <div className={styles.forms}>
                    ...
                    <div className={styles.userInfo}>
                        ...
                        <form className={styles.form} onSubmit={handleSubmitPassForm(changePasswordSubmit)}>
                            <Input
